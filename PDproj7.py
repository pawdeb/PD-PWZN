...
# python PDproj7.py 100 100 5 5 8 100
...
def ising (model, ile, a, b, beta):
    ...
    for i in range(ile):
        for k in range(a*b):
            ...
            E0 = (-j*org*sas1) + (-j*org*sas2) + (-j*org*sas3) + (-j*org*sas4)
            flip = org*(-1)
            #E1 = np.sum([(-j*flip*sas1), (-j*flip*sas2), (-j*flip*sas3), (-j*flip*sas4)])
            E1 = (-j*flip*sas1) + (-j*flip*sas2) + (-j*flip*sas3) + (-j*flip*sas4)
            dE = E1-E0
            if dE < 0:
                model[r1][r2] = flip
            else:
                ...
                r3 = np.random.randint(1000)/1000
                if r3 < p:
                    model[r1][r2] = flip

t1 = time.time()
ising(model,ile,a,b,beta)
t2 = time.time()
print(t2-t1)

klatka = Wizualizator(model)
klatka.save(os.path.dirname(os.path.realpath(__file__)) + '\\' + str(datetime.datetime.now().strftime("%Y-%m-%d %H-%M-%S")) + '.png')
